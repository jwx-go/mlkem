...
- `"z"`: base64url-encoded implicit-rejection value (private keys only, companion-private extension)
...
### Private Key Seed Size: 32-byte `priv` + Private `z` Extension
...
**Store 32 bytes in `"priv"` (the `d` component) per the draft, and store the
32-byte implicit-rejection value in a companion-private `"z"` field.**

Behavior:

| Scenario | Stored fields | On export |
|---|---|---|
| Private key emitted by this module | `priv = d`, `z = z` | Reconstruct exact stdlib `d \|\| z` seed |
| Public key only | `pub` only | Construct `EncapsulationKey` from `pub` |
| Legacy private JWK without `z` | `priv = d` only | Derive deterministic fallback `z = HKDF-SHA256(d, "jwx-go/mlkem z-derivation/<alg>")` |

Rationale:

1. **Keep the draft-visible field shape**: `"priv"` remains the 32-byte `d` value required by the current draft.
2. **Preserve exact key identity when possible**: private JWKs produced by this module now round-trip to the same 64-byte stdlib seed.
3. **Do not leak `z` through public JWKs**: the paired core `jwk` change strips `"z"` during AKP `PublicKey()` projection, so only private JWKs retain it.
4. **Keep legacy JWKs usable**: old ML-KEM JWKs without `"z"` still import and export, but the reconstructed key uses a deterministic compatibility `z` rather than a fresh random one.

**Legacy compatibility note:**

The deterministic fallback path exists only for ML-KEM JWKs that predate the
private `"z"` field. These reconstructed keys preserve the public key and make
implicit rejection stable across processes/restarts, but they cannot recover
the original random `z` unless the JWK already stored it. The fallback
therefore prioritizes restart stability over exact recovery for historical
payloads.
...
- For `*mlkem.DecapsulationKey768`: `seed := dk.Bytes()` → `priv = seed[:32]` (`d`),
  `z = seed[32:]`, `pub = dk.EncapsulationKey().Bytes()`
...
- Private key export: if JWK has `z`, build `seed = priv || z` (64 bytes) exactly;
  otherwise derive deterministic fallback `z` from `priv` + bare alg, then call
  `mlkem.NewDecapsulationKey768(seed)`
...
3. **JWK serialization**: Marshal/unmarshal ML-KEM keys, verify `"kty": "AKP"`, `"pub"`, `"priv"`, and private `"z"` fields
4. **Key import/export**: `jwk.Import(*mlkem.DecapsulationKey768)` → JWK → `jwk.Export[*mlkem.DecapsulationKey768]` → verify the full 64-byte seed matches
5. **Legacy compatibility**: remove `"z"` from a private JWK, parse twice, export twice, verify the reconstructed seeds are deterministic and the encapsulation key still matches
6. **Header `"ek"` field**: Verify KEM ciphertext appears in serialized JWE, round-trips correctly
7. **Direct mode**: Verify JWE Encrypted Key is absent in compact serialization for `ML-KEM-768` / `ML-KEM-1024`
8. **Key-wrap mode**: Verify JWE Encrypted Key is present for `ML-KEM-768+A192KW` / `ML-KEM-1024+A256KW`
9. **Algorithm-key mismatch**: Verify errors when wrong key type is provided
10. **KMAC256 test vectors**: Validate KMAC256 implementation against NIST SP 800-185 test vectors
11. **Interop vectors**: Once available from other implementations or the draft, add test vectors
